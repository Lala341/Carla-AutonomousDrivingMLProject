...
class CarlaDataCollector:
    ...
    def __init__(self, host="0.0.0.0", port=2000, 
        ...
        try:
            ...
            # Get the closest waypoint to the starting location
            carla_map=self.world.get_map()
            start_waypoint = carla_map.get_waypoint(lap_start_wp.transform.location)
            waypoints = [start_waypoint]
            num_waypoints = 10000

            for _ in range(num_waypoints):
                next_waypoints = start_waypoint.next(1.0)  # You can adjust the lane width (1.0 in this example)

                # Find the nearest waypoint to the current position
                nearest_waypoint = min(next_waypoints, key=lambda waypoint: waypoint.transform.location.distance(lap_start_wp.transform.location))

                waypoints.append(nearest_waypoint)
                start_waypoint = nearest_waypoint
    ...
    def save_observation(self):
        ...
        #print( "Progress: %.2f%%" % (self.num_saved_observations / self.num_images_to_save * 100.0))
    ...
    def get_step_deterministic(self, ind):     
        if self.is_done():
            raise Exception("Step called after CarlaDataCollector was done.") 
        self.vehicle.set_transform(self.waypoints[ind].transform)
        # Tick game
        self.clock.tick()
        self.hud.tick(self.world, self.clock)
        self.world.tick()
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--host", default="0.0.0.0", type=str, help="IP of the host server (default: 127.0.0.1)")
    ...
    #Deterministic way to get a route
    for i in range(10000):
        data_collector.get_step_deterministic(i)
