...
class SAC2PolicyGraph():
    """
    Manages the policy computation graph for Soft Actor-Critic
    """
    ...
    def __init__(self, input_states, action_space, scope_name,
        ...
        action_space (gym.spaces.Box):
            Continuous action space of our agent
        scope_name (string):
        ...
        hidden_sizes (list):
            List of layer sizes used to construct the policy network
        """
        ...
        with tf.variable_scope(scope_name):
            ...
            self.policy = build_mlp(input_states, hidden_sizes=hidden_sizes, activation=tf.nn.relu)
            self.action_mean = tf.layers.dense(self.policy, num_actions,
            ...
            self.value = build_mlp(input_states, hidden_sizes=hidden_sizes, activation=tf.nn.relu)
            self.value = tf.layers.dense(self.value, 1, activation=None, name="value")
            ...
            # log π(a_t | s_t; θ)
            self.action_log_prob = tf.reduce_sum(self.action_normal.log_prob(self.sampled_action), axis=-1, keepdims=True)



class SAC2():
    """
    Soft Actor-Critic model class
    """

    def __init__(self, input_shape, action_space,
                 learning_rate=3e-4, lr_decay=0.998,
                 initial_std=0.4, alpha=0.2,
                 hidden_sizes=(256, 256),
                 model_dir="./"):
        """
        input_shape [3]:
        ...
        self.q_values = tf.placeholder(shape=(None,), dtype=tf.float32, name="q_values_placeholder")
        self.advantage = tf.placeholder(shape=(None,), dtype=tf.float32, name="advantage_placeholder")
        

        # Create policy graphs
        self.policy = SAC2PolicyGraph(self.input_states, action_space, "policy", initial_std=initial_std, hidden_sizes=hidden_sizes)

        
        # Entropy regularization term
        self.entropy_loss = -alpha * tf.reduce_sum(self.policy.action_normal.entropy(), axis=-1)


                # # Entropy loss
        # self.entropy_loss = -tf.reduce_sum(self.policy.action_normal.entropy(), axis=-1)
        ...
        # Update network parameters #just added
    ...
    def compute_q_values(self, input_states):
        # Ensure input_states has a batch dimension
        if len(input_states.shape) == 1:
            input_states = np.expand_dims(input_states, axis=0)

        q_values = self.sess.run(self.policy.value, feed_dict={self.input_states: input_states})
        return q_values.flatten()


    # ... (the rest of the class methods
    # are the same as in the PPO class)
    ...
    def train(self, input_states, taken_actions, returns, advantage):#, q_values):
        _, _, summaries, step_idx = \
            self.sess.run([self.train_step, self.update_metrics_op, self.stepwise_summaries, self.train_step_counter.var],
                feed_dict={
                   ...
                   # self.q_values: q_values  # Pass the state-action values
