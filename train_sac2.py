...
def train(params, start_carla=True, restart=False):
    ...
    model = SAC2(input_shape, env.action_space,
                learning_rate=learning_rate, lr_decay=lr_decay,
                initial_std=initial_std, alpha=entropy_scale,
    ...
    while num_episodes <= 0 or model.get_episode_idx() < num_episodes:
        ...
        while not terminal_state:
            states, taken_actions, values, rewards, dones, q_values_list = [], [], [], [], [], []
            ...
            advantages = cadvantages(q_values_list[:T], values[:T])
            # Pad advantages to have the same length as other arrays
            #advantages = np.pad(advantages, (0, horizon - T), mode='constant')
            ...
            print(returns.shape)
            returns
            if(len(advantages.shape)>1 and advantages.shape[1]!=1):
            ...
            for _ in range(num_epochs):
                ...
                for i in range(int(np.ceil(num_samples / batch_size))):
                    ...
                    model.train(states[mb_idx], taken_actions[mb_idx],
                                returns[mb_idx], advantages[mb_idx])#, q_values_list[mb_idx])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--entropy_scale", type=float, default=0.4, help="Entropy loss scale factor")#0.01
