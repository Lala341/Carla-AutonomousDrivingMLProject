...
def train(params, start_carla=True, restart=False):
    ...
    your_buffer_size = 10000
    your_noise_std = 0.1

    
    model = DDPG(input_shape, env.action_space, buffer_size=your_buffer_size, 
                 noise_std=your_noise_std, model_dir=os.path.join("models", model_name))
    ...
    while num_episodes <= 0 or model.get_episode_idx() < num_episodes:
        ...
        while not terminal_state:
            ...
            # Set the stop time
            training_time = datetime.datetime.now() + datetime.timedelta(hours=2)  # Stop after 2 hours

            # Train for some number of epochs
            model.update_old_policy() # θ_old <- θ
            for _ in range(num_epochs):
                # Check the time
                if datetime.datetime.now() > training_time:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--training_time", type=int, default=0, help="Number of times in hours to train for (0 or less trains forever)")  
